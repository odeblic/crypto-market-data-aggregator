...
> A market data aggregator that connects to several crypto exchanges,
> consolidates quotes into a single book, and publishes it internally for
> consumption by various clients.
...
| Folder      | Content                                           |
| ----------- | ------------------------------------------------- |
| `prototype` | exchange connectors written in Python             |
| `doc`       | documentation and screenshots                     |
| `cmake`     | scripts used by CMake for self-contained builds   |
| `src`       | application code that actually goes to production |
| `proto`     | gRPC definitions for the market data protocol     |
| `test`      | code for all unit tests                           |
...
Surprises are not uncommon and for this reason, it is advisable to
explore the CEX part by prototyping the connectivity in Python.

The purpose is to explore the API of each exchange and discover the data.

Only after being able to collect unified market data can the C++ implementation
begin on safe ground.

The code can be found in [prototype/exchange-connectivity](./prototype/exchange-connectivity/) with its documentation.

## Design and architecture

The aggregator spawns three threads in addition to the main thread for the following tasks:

1. dispatch the market data to logging and aggregation
2. generate book snapshots
3. serve the data with gRPC

A lock-free queue is used for market data updates. All exchange connectors publish to it and the aggregation component consumers from it.

Market data is published as snapshots sent every second. This greatly simplifies both the connectivity and the gRPC-based protocol.

If this project were to scale up, a diff-based protocol would have to be implemented (it is already described in the `.proto` files), implying:

+ a mechanism to trigger updates inside the aggregator
+ a book building logic on client side
+ a snapshot and recovery for (re-)connections on both sides

### External libraries

+ [Boost](https://www.boost.org) for utils (lockfree queue, asio, etc.)
+ [Beast](https://www.boost.org/doc/libs/latest/libs/beast/doc/html) for websockets handling
...
The internal exchange requires SSL private key and certificate.

```sh
# use openssl to generate the files...
openssl req -x509 -newkey rsa:2048 -keyout private-key.pem -out certificate.pem -days 365 -nodes -subj "/CN=localhost"

# or the helper Makefile to do it for you!
make ssl
```
